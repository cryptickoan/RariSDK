                    ...
                    const contractBalanceUsdBN = contractBalanceBN
                        ...
                        .div(
                            internalTokens[currencyCode].decimals === 18 
                                ...
                                : BigNumber.from(10 ** internalTokens[currencyCode].decimals)
                        );
        ...
        this.deposits = {
              ...
              // First with mStable (if currencyCode is supported), then with 0xSwap
                          ...
                          if ( acceptedCurrency === "mUSD" || MStableSubpool.SUPPORTED_EXCHANGE_CURRENCIES.indexOf(acceptedCurrency) >= 0) {
                              ...
                              // If currency we want to deposit is not mUSD but its still supported by mStable exchange
                                  ...
                                  .mul(
                                      ...
                                      ? constants.WeiPerEther
                                      : BigNumber.from(10 ** internalTokens[acceptedCurrency].decimals)
                                  )
                                  .div(
                                      internalTokens[currencyCode].decimals >= 18 
                                      ? constants.WeiPerEther
                  ...
                  if (mStableOutputCurrency !== null) {
                      ...
                      const outputAmountUsdBN = mStableOutputAmountAfterFeeBN
                          .mul(
                              BigNumber.from(allBalances["4"][self.allocations.CURRENCIES.indexOf(mStableOutputCurrency)])
                          )
                          .div(
                              (BigNumber.from(10)).pow(internalTokens[mStableOutputCurrency].decimals)
                          )
                      ...
                      var makerAssetFilledAmountUsdBN = makerAssetFilledAmountBN.mul(
                          BigNumber.from(allBalances["4"][self.allocations.CURRENCIES.indexOf(acceptedCurrency)])
                      ).div(
                          BigNumber.from(10).pow(BigNumber.from(internalTokens[acceptedCurrency].decimals))
                      );
                      ...
                      var ethBalanceBN = currencyCode === "ETH" 
                          ? accountBalanceBN
                          : BigNumber.from(await provider.getBalance(sender))
          ...
          getDepositSlippage: async function (currencyCode, amount, usdAmount) {
              ...
              if (self.cache._raw.coinGeckoUsdPrices 
                  ...
                  return constants.WeiPerEther.sub(BigNumber.from(
                      Math.trunc(
                  ...
                  if (mStableOutputCurrency !== null) {
                      ...
                      try {
                          var allowanceBN = BigNumber.from(
                              await internalTokens[currencyCode].contract.callStatic.allowance(sender, self.contracts.RariFundProxy.options.address)
                          );
                      ...
                      try {
                          var receipt = await self.contracts.RariFundProxy.exchangeAndDeposit(currencyCode, amount, mStableOutputCurrency, sender);
                      } catch (err: any) {
                      ...
                      currencyCode == "ETH"
                      ? accountBalanceBN
                      : BigNumber.from(await self.provider.getBalance(sender));
      ...
      this.withdrawals = {
          ...
          getMaxWithdrawalAmount: async function (currencyCode, senderUsdBalance, sender) {
                    ...
                    maxInputAmountLeftBN
                      ...
                      .div(
                        BigNumber.from(10)
                          .pow(
                            BigNumber.from(
                              internalTokens[inputCandidate.currencyCode]
                                .decimals
                            )
                          )
                      )
                ...
                for (var i:any = 0; i < inputCandidates.length; i++) {
                  ...
                  if (inputCandidates[i].currencyCode === "mUSD") {
                    ...
                    var outputAmountBeforeFeesBN = mStableInputAmountBN
                      .mul(
                        BigNumber.from(10).pow(BigNumber.from(internalTokens[currencyCode].decimals))
                      )
                      .div(
                        BigNumber.from(10).pow(BigNumber.from(internalTokens[inputCandidates[i].currencyCode].decimals))
                      );
              ...
              if (
                ...
                for (var i:any = 0; i < inputCandidates.length; i++) {
                  ...
                  if (
                    inputFillAmountUsdBN.gte(usdAmountLeft.sub(BigNumber.from(10).mul(BigNumber.from(16))))
                  ) {
          ...
          validateWithdrawal: async function (
              ...
              if (tokenRawFundBalanceBN.gte(amount)) {
                  var amountUsdBN = amount
                      .mul(
                          BigNumber.from( allBalances["4"][self.allocations.CURRENCIES.indexOf(currencyCode)])
                      )
                      .div(
                          BigNumber.from(10).pow(BigNumber.from(internalTokens[currencyCode].decimals))
                      )
                        ...
                        currencyCode
                          ...
                          if (currencyCode !== "mUSD" && mStableSwapFeeBN === null) {
                              ...
                              var inputAmountBN = amount
                                  ...
                                  .mul(
                                      internalTokens[inputCandidates[i].currencyCode].decimals === 18 
                                      ? constants.WeiPerEther 
                                      : BigNumber.from( 10 ** internalTokens[inputCandidates[i].currencyCode].decimals) 
                                  ).div(
                              ...
                              while (outputAmountBN.lt(amount.sub(amountWithdrawnBN))) {
                                  ...
                                  outputAmountBeforeFeesBN = inputAmountBN
                                    ...
                                    .div(
                                      BigNumber.from(10).pow(internalTokens[inputCandidates[i].currencyCode].decimals)
                                      );
                                  outputAmountBN =
                                    currencyCode === "mUSD"
                                      ? outputAmountBeforeFeesBN
                  ...
                  amount
                    ...
                    .mul(
                      BigNumber.from(10)
                        .pow(
                          BigNumber.from(
                            36 - internalTokens[currencyCode].decimals
                          )
                        )
                    )
                    .div(usdAmount)
          ...
          withdraw: async function (currencyCode, amount, maxUsdAmount, options) {
            ...
            if (tokenRawFundBalanceBN.gte(amount)) {
              ...
              try {
                var receipt = await self.contracts.RariFundManager
              ...
              // Otherwise, exchange as few currencies as possible (ideally those with the lowest balances)
              let inputCurrencyCodes: any[] = [];
              let inputAmountBNs: any[]  = [];
              let allOrders: any[]  = [];
              let allSignatures: any[]  = [];
              let makerAssetFillAmountBNs: any[]  = [];
              let protocolFeeBNs: any[]  = [];
                ...
                for (var i:any = 0; i < inputCandidates.length; i++) {
                  ...
                  if (inputAmountBN.gt(inputCandidates[i].rawFundBalanceBN)) {
                    ...
                    outputAmountBeforeFeesBN = inputAmountBN
                      .mul(BigNumber.from(10).pow(BigNumber.from(allTokens[currencyCode].decimals)))
                      .div(
                        BigNumber.from(10).pow(BigNumber.from(
                            internalTokens[inputCandidates[i].currencyCode]
                              .decimals
                        ))
                      );
                  ...
                  if (inputCandidates[i].currencyCode === "mUSD") {
                    try {
                      var redeemValidity = await self.pools[
                        "mStable"
                      ].externalContracts.MassetValidationHelper.callStatis
                        .getRedeemValidity(
                          "0xe2f2a5c287993345a840db3b0845fbc70f5935a5",
                          inputAmountBN,
                          internalTokens[currencyCode].address
              ...
              try {
                let inputAmountStrings: any = [];
                for (var i:any = 0; i < inputAmountBNs.length; i++)
                  inputAmountStrings[i] = inputAmountBNs[i].toString();
                let makerAssetFillAmountStrings: any[] = [];
                for (var i:any = 0; i < makerAssetFillAmountBNs.length; i++)
                  makerAssetFillAmountStrings[i] = makerAssetFillAmountBNs[
                    i
                  ].toString();
                let protocolFeeStrings: any[] = [];
                for (var i:any = 0; i < protocolFeeBNs.length; i++)
                  protocolFeeStrings[i] = protocolFeeBNs[i].toString();
                var receipt = await self.contracts.RariFundProxy
                  .withdrawAndExchange(
                    inputCurrencyCodes,
                    inputAmountStrings,
                    currencyCode === "ETH"
                    ...
                    protocolFeeStrings,
                    {
                      ...
                      gasPrice: gasPrice,
                      nonce: await self.provider.getTransactionCount(options.from),
                    }
                  )
              } catch (err: any) {
                throw new Error(
