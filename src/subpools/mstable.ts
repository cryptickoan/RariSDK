...
for (const contractName of Object.keys(externalContractAddressesMStable)) {
  externalAbisMStable[contractName] = require("./mstable/abi/" +
    contractName +
    ".json");
}
...
export default class mStableSubpool {
  ...
  async getMUsdSavingsApy(includeIMUsdVaultApy) {
    ...
    this.cache.update(
      "mUsdSwapFee",
      ethers.BigNumber.from(data.data.masset.feeRate)
    );

    let apy = ethers.utils.parseUnits(
  ...
  async getIMUsdVaultApy(totalStakingRewards, stakingTokenPrice) {
    const weeklyROI = await this.getIMUsdVaultWeeklyRoi(
      totalStakingRewards,
      stakingTokenPrice
    );
    const apyBN = ethers.utils.parseUnits(
      ethers.utils.formatUnits(
        (((weeklyROI + 1) ** 52 - 1) * 1e18).toFixed(0),
        18
      ),
      18
    );
