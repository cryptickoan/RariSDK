export default class Cache {
  ...
  constructor(timeouts) {
    for (const key of Object.keys(timeouts)) this._raw[key] = { value: null, lastUpdated: 0, timeout: timeouts[key] };
  }

  async getOrUpdate(key, asyncMethod) {
    let now = new Date().getTime() / 1000;

    if (this._raw[key].value == null || now > this._raw[key].lastUpdated + this._raw[key].timeout) {
      const self = this;
